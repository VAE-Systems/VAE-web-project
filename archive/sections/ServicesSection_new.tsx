...
const ServicesSection: React.FC = () => {
  ...
  return (
    <section
      id="services"
      ref={sectionRef}
      className="relative overflow-hidden border-t border-vae-turquoise/10 bg-gradient-to-br from-bg-darker via-bg-dark to-bg-secondary py-24 md:py-36"
    >
      {/* Background Effects */}
        ...
        <div className="mt-8 space-y-1 text-[10px] text-text-muted">
          {Object.entries(footnotes).map(([key, text]) => (
            <div key={key} className="flex items-start gap-2">
              <sup className="mt-[-2px] text-vae-turquoise/60">{superscripts[key]}</sup>
              <span>{text}</span>
            </div>
          ))}
        </div>
