...
const ValuesPrinciplesSection: React.FC<ValuesPrinciplesSectionProps> = ({ className }) => {
  ...
  return (
    <section
      id="werte"
      className={className ? className : 'section-block mb-40'}
      data-section
      ref={wrapperRef}
      aria-labelledby="werte-heading"
    >
      <div className="mx-auto max-w-6xl">
        <header className="mb-20 max-w-3xl" data-animate>
          <h2 id="werte-heading" className="fluid-h2 heading-fix mb-6 font-bold text-white">
            Leitprinzipien
          </h2>
          <p className="text-lg leading-relaxed text-text-secondary md:text-xl">
            Haltung vor Hype: Substanz, Nachvollziehbarkeit und nachhaltiger Betrieb. Jedes Projekt unterliegt denselben
            Standards – unabhängig von Größe oder Laufzeit.
          </p>
        </header>
        <div className="relative pl-10 md:pl-14">
          ...
          <div
            aria-hidden="true"
            className="absolute bottom-0 left-2 top-0 w-px overflow-hidden bg-gradient-to-b from-vae-turquoise/0 via-vae-turquoise/70 to-vae-turquoise/0 md:left-4"
          >
            <div
              data-timeline-line
              className="absolute inset-0 bg-gradient-to-b from-vae-turquoise via-vae-turquoise to-vae-turquoise"
            />
          </div>
          <ol className="space-y-20">
            {principles.map((p, idx) => (
              <li key={p.title} data-timeline-item className="relative">
                {/* Node */}
                <div aria-hidden="true" className="absolute -left-[38px] top-1.5 md:-left-[46px]">
                  <div className="timeline-node flex h-8 w-8 items-center justify-center rounded-full border border-vae-turquoise/40 bg-vae-turquoise/20 backdrop-blur-sm">
                    <Icon name={p.icon} className="text-vae-turquoise" size={16} />
                  </div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur-sm transition-colors hover:border-vae-turquoise/40 md:p-8">
                  <div className="mb-4 flex items-start justify-between">
                    <h3 className="pr-6 text-xl font-semibold leading-snug text-white">{p.title}</h3>
                    <span className="text-xs font-medium tracking-wider text-vae-turquoise/70">
                      {String(idx + 1).padStart(2, '0')}
                    </span>
                  </div>
                  <p className="max-w-2xl text-sm leading-relaxed text-text-secondary md:text-base">{p.body}</p>
                </div>
              </li>
            ))}
          </ol>
