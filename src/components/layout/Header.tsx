...
const Header: React.FC = () => {
  ...
  const headerSurface = isDarkMode
    ? isScrolled
      ...
      : 'border-white/10 bg-bg-darker/70 shadow-[0_48px_96px_-52px_rgba(8,12,24,0.85)]'
    : isScrolled
      ? 'border-white/70 bg-white/85 shadow-[0_28px_60px_-30px_rgba(15,23,42,0.25)]'
      : 'border-white/60 bg-white/65 shadow-[0_42px_82px_-46px_rgba(15,23,42,0.25)]'
  ...
  const navFloatingClass = [
    ...
    isDarkMode
      ...
      : isScrolled
        ? 'border-white/70 shadow-[0_24px_60px_-34px_rgba(15,23,42,0.25)]'
        : 'border-white/60 shadow-[0_32px_74px_-36px_rgba(15,23,42,0.25)]',
    isScrolled ? 'scale-[0.985]' : 'scale-100',
  ].join(' ')

  const themeButtonClass =
    'inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/60 bg-white/70 text-text-primary shadow-sm transition-all duration-300 hover:border-vae-turquoise hover:text-vae-turquoise dark:border-white/15 dark:bg-white/[0.08] dark:text-white'
    ...
    <header className={headerClassName}>
      <div className="hidden md:block">
        <div className="container-vae">
          ...
          >
            ...
            <Link
              to="/kontakt"
              className="text-text-primary inline-flex items-center gap-2 rounded-full border border-white/50 px-4 py-2 text-xs font-semibold transition-colors hover:border-vae-turquoise hover:text-vae-turquoise dark:border-white/20 dark:text-white"
            >
      ...
      <div className="container-vae">
        <div className="flex items-center justify-between gap-3 py-3 md:grid md:h-[96px] md:grid-cols-[auto,1fr,auto] md:items-center md:gap-6 md:py-0">
          <Link
            to="/"
            className="dark:border-white/12 group flex items-center gap-4 rounded-2xl border border-white/60 bg-white/70 px-3 py-2 transition-all duration-300 hover:border-vae-turquoise/40 hover:bg-white/80 dark:bg-white/[0.06] md:px-4"
          ...
          <div className="flex items-center gap-2 md:justify-end md:gap-3">
            <div className="hidden items-center gap-3 md:flex">
              ...
              <CtaLink
                ...
                className="text-text-primary dark:border-white/12 inline-flex items-center gap-2 rounded-full border border-white/60 bg-white/75 px-5 py-2 text-sm font-semibold shadow-[0_18px_44px_-28px_rgba(15,23,42,0.35)] transition-all duration-300 hover:border-vae-turquoise hover:text-vae-turquoise dark:bg-white/[0.08] dark:text-white"
            ...
            <button
              className={`${themeButtonClass} md:hidden`}
              onClick={toggleTheme}
              aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}
            >
              {theme === 'dark' ? <Sun className="h-5 w-5" /> : <Moon className="h-5 w-5" />}
            ...
            <button
              ...
              className="text-text-primary inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/60 bg-white/70 shadow-sm transition-all duration-300 hover:border-vae-turquoise hover:text-vae-turquoise active:scale-[0.98] dark:border-white/15 dark:bg-white/[0.08] dark:text-white md:hidden"
      ...
      {mobileMenuVisible && (
        <>
          ...
          >
            <div
              className={`border-border-primary flex h-full transform flex-col border-t bg-white/95 shadow-2xl transition-transform duration-300 ease-out dark:border-vae-turquoise/30 dark:bg-bg-darker/95 dark:shadow-black/40 ${
                  ...
                  {navigation.map(item => {
                    ...
                    if (item.dropdown?.length) {
                      ...
                      return (
                        <div
                          key={item.label}
                          className="rounded-2xl border border-white/40 bg-white/70 shadow-sm dark:border-white/10 dark:bg-white/[0.04]"
                        >
                          <button
                            type="button"
                          ...
                          {expanded && (
                            <div
                              id={mobileDropdownId}
                              className="space-y-3 border-t border-white/40 px-5 py-4 dark:border-white/10"
                            >
                              {item.dropdown.map(subItem => (
                                <Link
                                  key={subItem.path}
