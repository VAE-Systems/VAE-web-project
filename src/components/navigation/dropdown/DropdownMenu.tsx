        ...
        return (
          ...
          >
            <button
              ...
              className={`group inline-flex items-center gap-2 px-1 py-2 text-[11px] font-black uppercase tracking-[0.28em] transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-vae-turquoise/50 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-offset-[#030806] ${
                panelVisible
                  ? 'text-vae-turquoise'
                  : 'text-text-secondary hover:text-text-light dark:text-white/80 dark:hover:text-white'
              }`}
              aria-haspopup="true"
              ...
              <ChevronDown
                className={`h-3.5 w-3.5 transition-transform duration-200 ${
                  panelVisible
                    ...
                    : 'text-text-secondary group-hover:text-text-light dark:text-white/50 dark:group-hover:text-white/80'
                }`}
              />
            </button>
            ...
            <div
              ...
              } overflow-hidden border shadow-[0_8px_40px_-8px_rgba(0,0,0,0.18)] transition-[height,opacity,transform] duration-300 ease-out min-[1150px]:w-[min(calc(100vw-2rem),44rem)] min-[1200px]:w-[min(calc(100vw-2rem),48rem)] xl:w-[min(calc(100vw-2rem),56rem)] ${
                panelVisible
                  ? 'pointer-events-auto translate-y-0 opacity-100'
                  : 'pointer-events-none -translate-y-2 opacity-0'
              } border-black/10 bg-white dark:border-white/10 dark:bg-[hsla(160,14%,6%,0.98)]`}
              style={{ height: panelHeight ? `${panelHeight}px` : undefined }}
            >
              <div ref={getPanelRefHandler(menu.id)}>
                {menu.subtitle && (
                  <div className="bg-bg-primary border-b border-black/10 px-6 py-3 dark:border-white/10 dark:bg-black/40">
                    <p className="text-[10px] font-black uppercase tracking-[0.3em] text-vae-turquoise">
                      {menu.subtitle}
                    </p>
                  </div>
                )}
  ...
  const renderMobileMenu = () => (
    <div className="w-full lg:hidden" aria-label="Mobile Navigation">
      <button
        ...
        className="flex w-full items-center justify-between rounded-xl border border-black/10 bg-white px-4 py-3 text-[11px] font-black uppercase tracking-[0.3em] text-text-light transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-vae-turquoise focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:border-white/15 dark:bg-white/5 dark:text-white dark:focus-visible:ring-offset-[#030806]"
      >
        <span>Menü</span>
      ...
      {isMobileMenuOpen && (
        <div className="bg-bg-primary mt-2 space-y-2 border border-black/10 p-4 text-text-light shadow-[0_8px_32px_-8px_rgba(0,0,0,0.12)] dark:border-white/10 dark:bg-bg-dark dark:text-white">
          {menus.map(menu => {
            ...
            return (
              <div
                key={menu.id}
                className="overflow-hidden border border-black/10 bg-white dark:border-white/10 dark:bg-white/5"
              >
                <button
                  type="button"
                  className="flex w-full items-center justify-between rounded-xl border border-transparent px-4 py-3 text-left text-[11px] font-black uppercase tracking-[0.2em] text-text-light transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-vae-turquoise focus-visible:ring-offset-2 focus-visible:ring-offset-white aria-expanded:border-vae-turquoise/25 aria-expanded:bg-vae-turquoise/5 dark:text-white dark:focus-visible:ring-offset-[#030806]"
                ...
                >
                  ...
                  <ChevronDown
                    className={`h-4 w-4 transition-transform duration-200 ${
                      isExpanded ? 'rotate-180 text-vae-turquoise' : 'text-text-secondary dark:text-white/60'
                    }`}
                  />
                </button>
                      ...
                      {menu.menuItems.map(item => (
                        <li key={item.id}>
                          <a
                            ...
                            className="block rounded-xl border border-transparent px-4 py-2.5 text-[11px] font-black uppercase tracking-[0.16em] text-text-light transition-all duration-150 hover:border-vae-turquoise/30 hover:bg-vae-turquoise/5 hover:text-vae-turquoise focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-vae-turquoise focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:text-white/70 dark:hover:border-vae-turquoise/30 dark:hover:text-white dark:focus-visible:ring-offset-[#030806]"
                          >
                            {item.label}
                          </a>
                        </li>
