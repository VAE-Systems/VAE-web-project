        ...
        <div className="container-vae max-w-5xl">
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-16 items-stretch">
            {[{h:'Integrationen',b:['API-Connectoren','Webhook Orchestrierung','Legacy Brücken','Event Normalisierung']},{h:'Automatisierung',b:['Workflow Implementierung','Human-in-the-loop Pfade','Kosten / Latenz Budgets','Fallback / Degradation']},{h:'Retrieval & Data',b:['Domain Embeddings','Index Strategie','Sicherheits-Scoping','Evaluation Hooks']}].map(c => (
              <div key={c.h} className="p-6 rounded-2xl bg-white/5 border border-white/10">
                ...
                <ul className="text-sm text-text-secondary space-y-1 leading-relaxed list-disc list-inside">{c.b.map(x => <li key={x}>{x}</li>)}</ul>
              </div>
            ))}
          </div>
          <div className="bg-white/5 border border-white/10 rounded-2xl p-8 mb-14">
            ...
            <div className="mt-4 p-4 rounded-xl bg-vae-turquoise/10 border border-vae-turquoise/30">
              <p className="text-sm text-text-secondary leading-relaxed"><span className="text-white font-medium">Hinweis:</span> Prüfen wir während Discovery, dass Ihr Anwendungsfall durch eine vorhandene <Link to="/products/solutions" className="underline text-vae-turquoise hover:text-white">Komplettlösung</Link> effizienter abbildbar ist, erhalten Sie eine klare Empfehlung & Aufwandseinschätzung für Adaption statt Neu‑Implementierung.</p>
            </div>
          </div>
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-10 items-stretch">
              {[{h:'Kern-KPIs',p:['Qualitäts-Score','Durchlaufzeit / Latenz','Kosten / Aufruf']},{h:'Guardrails',p:['Fehlerraten','Token / Ressourcen Budget','Security / Zugriff']},{h:'Observability',p:['Dashboards','Alerting Schwellen','Drilldown Queries']}].map(b => (
                <div key={b.h} className="p-6 rounded-2xl bg-white/5 border border-white/10">
                  ...
                  <ul className="text-sm text-text-secondary space-y-1 leading-relaxed list-disc list-inside">{b.p.map(x=> <li key={x}>{x}</li>)}</ul>
                </div>
              ))}
            </div>
              ...
              {productCategories.map(cat => {
                ...
                return (
                  <SpotlightCard
                    ...
                    cta={cat.cta}
                    className="p-0"
                    iconSlot={<div className="px-2 py-1 rounded-md bg-vae-turquoise/10 text-vae-turquoise text-xs font-semibold inline-flex items-center border border-vae-turquoise/30">{cat.badge || 'Suite'}</div>}
                  >
          ...
          {lifecycleSteps.map((s,i) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4 relative z-10">
                <span className="px-2 py-1 rounded-md bg-vae-turquoise/10 text-vae-turquoise text-xs font-semibold tracking-wider">{String(i+1).padStart(2,'0')}</span>
                <span className="text-xs text-vae-turquoise/70 font-medium uppercase tracking-wide">Phase</span>
              </div>
              ...
              <p className="text-xs text-text-secondary leading-relaxed relative z-10 flex-grow">{s.desc}</p>
              <div className="mt-4 pt-3 border-t border-white/10 text-xs text-text-muted relative z-10">
