...
export const LeaderProfileSection: React.FC<LeaderProfileSectionProps> = ({ leader, alignment = 'left' }) => {
  ...
  const impactLine = useMemo(() => {
    const words = leader.whyItMatters.split(' ')
    if (words.length <= 26) return leader.whyItMatters
    ...
    >
      <div className="container-vae grid gap-12 lg:grid-cols-[1.15fr_0.95fr] lg:items-center">
        <div className={`space-y-6 ${infoColumnOrder}`}>
          ...
          <h2 id={`${leader.id}-title`} className="text-3xl font-semibold text-white md:text-4xl lg:text-5xl">
            {leader.name} — {leader.title}
          </h2>
          <span className="inline-flex items-center rounded-full border border-white/10 px-4 py-1 text-[0.7rem] font-semibold uppercase tracking-[0.3em] text-white/70">
            {leader.roleTag}
          </span>
          ...
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {leader.characterTraits.map(trait => (
                <span
                  key={trait}
                  className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1 text-xs font-medium text-white/75 transition duration-300 hover:-translate-y-0.5 hover:border-vae-turquoise/60"
                >
                  {trait}
                </span>
              ))}
          ...
          <div className="space-y-4">
            <p className="text-sm text-white/60">Standort: {leader.location}</p>
            <MagneticButton intensity={0.08} className="w-full sm:w-fit">
              <a
                href={leader.linkedin.href}
                target="_blank"
                rel="noreferrer noopener"
                aria-label={leader.linkedin.text}
                className="btn-secondary inline-flex items-center justify-center gap-2 px-6 py-3 text-sm font-semibold uppercase tracking-[0.3em]"
              >
            ...
            <div className="flex flex-wrap gap-2">
              {leader.seoKeywords.map(keyword => (
                <span
                  key={keyword}
                  className="rounded-full border border-white/15 px-3 py-1 text-[11px] uppercase tracking-[0.2em] text-white/60 transition duration-300 hover:-translate-y-0.5 hover:border-vae-turquoise/60 hover:text-white"
                >
                  {keyword}
                </span>
              ))}
        ...
        <aside className={`flex flex-col gap-6 ${cardColumnOrder}`}>
          <figure className="group relative overflow-hidden rounded-[32px] border border-white/10 bg-white/[0.04] shadow-[0_18px_50px_rgba(7,15,25,0.55)]">
            <picture>
              ...
              <img
                ...
                loading="lazy"
                className="h-[520px] w-full object-cover object-top transition duration-[900ms] ease-out group-hover:translate-y-2 group-hover:scale-[1.08] sm:h-[580px]"
              />
            </picture>
            ...
            <div className="absolute inset-x-0 bottom-0 flex flex-col gap-2 px-6 pb-6 text-white">
              <p className="text-[0.65rem] font-semibold uppercase tracking-[0.35em] text-vae-turquoise/80">
                Signature Move
              </p>
              <p className="text-lg font-medium text-white/90">{impactLine}</p>
            </div>
            <figcaption className="sr-only">{leader.portrait.alt}</figcaption>
          </figure>

          <div className="rounded-[32px] border border-dashed border-white/15 bg-white/[0.03] px-6 py-4 text-xs text-white/60 shadow-[0_30px_80px_rgba(0,0,0,0.35)]">
