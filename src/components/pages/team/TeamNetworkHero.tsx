/**
 * Team Network Hero Section
 * Design identisch zu LeadershipHero
 */
import { TEAM_NETWORK_HERO } from '@/content/shared/teamNetworkData'
import React, { useMemo } from 'react'

export const TeamNetworkHero: React.FC = () => {
  const bodyParagraphs = useMemo(() => TEAM_NETWORK_HERO.body.split(' — ').map(chunk => chunk.trim()), [])

  return (
    <section className="relative z-0 overflow-hidden border-b border-black/5 bg-bg-darker py-28 text-text-light dark:border-white/5">
      ...
      <div className="container-vae relative">
        <div className="mx-auto flex max-w-4xl flex-col gap-6 text-center">
          ...
          <h1 className="text-balance text-4xl font-semibold leading-tight md:text-5xl lg:text-6xl">
            {TEAM_NETWORK_HERO.title}
          </h1>
          <p className="max-w-2xl text-lg font-medium text-text-secondary md:text-xl">{TEAM_NETWORK_HERO.subheading}</p>
          ...
          <div className="flex flex-wrap gap-2 pt-4" aria-label="SEO Schlagworte">
            {TEAM_NETWORK_HERO.seoKeywords.map(keyword => (
              <span
                key={keyword}
