...
export const VisionSection: React.FC = () => {
  ...
  return (
    <section
      ...
      className="relative overflow-hidden border-t border-white/5 bg-gradient-to-b from-bg-darker via-[#081013] to-bg-darker py-32"
    >
          ...
          {visionIcons.map((icon, i) => (
            ...
            >
              <div className="flex h-48 w-48 items-center justify-center rounded-full border border-vae-turquoise/20 bg-vae-turquoise/10 opacity-25 blur-2xl dark:border-vae-turquoise/25 dark:bg-vae-turquoise/20 md:h-64 md:w-64">
                <Icon name={icon.name} className="text-white/70" size={120} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="container-vae relative z-10 flex flex-col items-center text-center">
        <div
          ref={contentRef}
          className="max-w-4xl space-y-8 rounded-[32px] border border-white/10 bg-gradient-to-br from-white/[0.05] via-white/[0.03] to-transparent px-8 py-12 shadow-[0_25px_70px_rgba(0,0,0,0.4)] backdrop-blur-xl md:px-12 md:py-16"
        >
          <p className="text-xs font-semibold uppercase tracking-[0.4em] text-vae-turquoise/70">Unsere Vision</p>
          <h2 id="vision-title" className="text-3xl font-semibold text-white md:text-4xl lg:text-5xl">
            {VISION_CONTENT.title}
          </h2>
          <blockquote className="border-l-4 border-vae-turquoise pl-6 text-left text-lg italic leading-relaxed text-white/90 md:text-xl">
            {VISION_CONTENT.quote}
          </blockquote>
          <p className="text-base leading-relaxed text-white/75 md:text-lg">{VISION_CONTENT.explanation}</p>
          ...
          <div className="mt-12 grid grid-cols-2 gap-4 md:grid-cols-4">
            {visionIcons.map(icon => (
              <div key={icon.name} className="flex flex-col items-center gap-2">
                ...
                <span className="text-sm font-medium text-white/80">{icon.label}</span>
              </div>
            ))}
          </div>
        </div>
