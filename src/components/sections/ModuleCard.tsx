...
import { ReModule } from '@/content/reSuite'

interface ModuleCardProps {
  mod: ReModule
}

const ModuleCard: React.FC<ModuleCardProps> = ({ mod }) => {
  return (
    <article
    ...
    >
      ...
      <ul className="space-y-2 mb-3 text-[13px] text-text-secondary">
        {mod.features.slice(0, 4).map((f) => (
          <li key={f} className="flex items-start gap-2">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="text-vae-turquoise flex-shrink-0">
      ...
      <div className="mt-auto flex items-center gap-3">
        ...
        <Link to={`/products/solutions/${mod.slug}`} className="btn-secondary">
          Details
        </Link>
      </div>
    </article>
