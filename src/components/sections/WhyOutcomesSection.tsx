      ...
      <div className="container-vae relative">
        <div className="max-w-6xl mx-auto">
          <header className="max-w-3xl mb-14" data-heading-accent>
            <h2 id="why-heading" className="text-4xl md:text-5xl font-bold text-white heading-fix mb-6">
              {tagline}
            </h2>
          ...
          <div className="grid lg:grid-cols-3 gap-12 items-start">
            ...
            <aside className="space-y-8" aria-label="Prinzipien & Beleg">
                ...
                <ul className="space-y-4">
                  {principles.map(p => (
                    <li key={p.key} className="flex items-start space-x-3" data-principle>
                      ...
                      <div>
                        <div className="text-white text-sm font-medium leading-tight">{p.title}</div>
              ...
              <div className="p-5 rounded-2xl border border-white/10 bg-white/[0.04] backdrop-blur-sm" data-principle>
                <blockquote className="text-sm text-text-secondary leading-relaxed mb-3">“{testimonial.quote}”</blockquote>
                <div className="text-xs text-white/70">
                  {testimonial.author && <span className="font-medium text-white">{testimonial.author}</span>} {testimonial.role && <> · {testimonial.role}</>} {testimonial.company && <> · {testimonial.company}</>}
                </div>
              </div>
            </aside>
          </div>
