  ...
  return (
    <article
      className={`group relative overflow-hidden rounded-none border transition duration-200 ${
        isOpen
          ? 'border-text-light/20 bg-white/90 shadow-[0_18px_45px_rgba(26,35,32,0.12)] dark:border-white/10 dark:bg-white/[0.05]'
          : 'border-text-light/10 bg-white/80 shadow-[0_10px_28px_rgba(26,35,32,0.08)] hover:border-text-light/20 dark:border-white/5 dark:bg-white/[0.03] dark:hover:border-white/10'
      }`}
    >
      <button
        type="button"
        ...
        className="flex w-full items-center justify-between px-6 py-5 text-left text-text-light transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-vae-turquoise/60 dark:text-white"
      >
        <span className="text-lg font-semibold text-text-light dark:text-white">{question}</span>
        {isOpen ? (
