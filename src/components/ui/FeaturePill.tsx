...
export const FeaturePill: React.FC<Props> = ({ icon, children, className = '', variant = 'neutral' }) => {
  ...
  const variantClasses =
    variant === 'gold'
      ? 'border-2 border-amber-400/50 bg-amber-400/10 text-amber-900 shadow-[0_8px_28px_-16px_rgba(212,175,55,0.18)] dark:border-amber-300/60 dark:bg-amber-300/12 dark:text-white'
      : variant === 'silver'
        ? 'border-2 border-slate-300/70 bg-slate-100/60 text-slate-900 dark:border-slate-400/50 dark:bg-white/5 dark:text-white'
