...
import {
  ...
  DESIGN_SYSTEM_EVENTS,
  SPACING_SCALE_DEFINITIONS,
  TYPOGRAPHY_SCALE_DEFINITIONS,
} from '../config/designSystem.constants'
...
class DesignSystemService {
  ...
  private generateTypographyScale(): Record<string, string> {
    ...
    Object.entries(TYPOGRAPHY_SCALE_DEFINITIONS).forEach(([key]) => {
      scale[key] = generateTypographyClamp(
        key as keyof typeof TYPOGRAPHY_SCALE_DEFINITIONS,
  ...
  private emitChange(): void {
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(
        new CustomEvent(DESIGN_SYSTEM_EVENTS.SCALE_CHANGED, {
